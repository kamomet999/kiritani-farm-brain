...
## 2026-07-19 — ハウス号数を「エリア記号＋連番」に再改定（第2版）

### 決定の核
- 通し番号01–63は廃止し、**エリア記号＋エリア内連番**に変更（ユーザー決定）。
  記号: KO=公民館 KJ=工場前 BA=梅花 OZ=おぞれ TA=高野 UW=上野西 UE=上野東 MI=宮枚 KA=垣内（命名規則: ローマ字頭2文字、頭文字重複時は2文字目で区別、上野は西=W/東=E）。
- 番号は各エリアの北端から。未設置3箇所は KJ04・TA01・TA02。通し番号01–63は内部番号としてデータに保持。
- 旧2体系（通し01–63、初版001–061）との対応表は `site-map/house-coordinates.md` に保持。
- 地図はラベルをピル型にし、矢印の引き出し線が番号の上を通らない描画に変更。
